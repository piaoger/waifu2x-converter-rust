...
use image::Image;
use model::{Layer, Model};
use std;

use simd::*;

pub const USE_SIMD: bool = true;
...
pub fn filter_cpu1(in_img: Image, model: &Model, perf: &mut PerfStatus) -> Image {
	...
	for layer in model.iter() {
				...
				for j in 0..layer.nInputPlane as usize {
					...
					for y in 0..new_height {
						...
						for x in 0..new_width {
							*out.get_unchecked_mut(off_dst + x) += *in_map.get_unchecked(off_src0 + x + 0) * w00
		...
		height = new_height;
		width_stride = width;
		perf.cnn_flo += ((layer.nInputPlane * layer.nOutputPlane * layer.kW * layer.kH * 2) as u64
			* (width * height) as u64) + (layer.nOutputPlane as u64 * width as u64 * height as u64);
...
pub fn filter_cpu2(in_img: Image, model: &Model, perf: &mut PerfStatus) -> Image {
	...
	for layer in model.iter() {
		...
		if layer.nOutputPlane < 32 {
			filter_cpu2_layer(
				layer,
				layer.nInputPlane as usize,
				layer.nOutputPlane as usize,
				width,
				height,
				stride,
				&mut temp,
	...
	unsafe {
		for y in 0..height {
			for x in 0..width {
				let in_off00 = y * stride + x * num_in;
				let in_off01 = in_off00 + num_in;
				let in_off02 = in_off00 + two_min_in;
				let in_off10 = in_off00 + stride;
				let in_off11 = in_off10 + num_in;
				let in_off12 = in_off10 + two_min_in;
				let in_off20 = in_off00 + stride * 2;
				let in_off21 = in_off20 + num_in;
				let in_off22 = in_off20 + two_min_in;
				for i in 0..num_in {
					let w = weights.get_unchecked(i);
					*input.get_unchecked_mut(0) = *buf.get_unchecked(in_off00 + i);
					*input.get_unchecked_mut(1) = *buf.get_unchecked(in_off01 + i);
						...
						if USE_SIMD {
							{
								let t0 = f32x4::new(
									...
									*temp.get_unchecked(j + 1),
									*temp.get_unchecked(j + 2),
									*temp.get_unchecked(j + 3),
								);

								let c0 = f32x4::new(
									convolve3x3(&input, &w, wi),
									convolve3x3(&input, &w, wi + 9),
									convolve3x3(&input, &w, wi + 18),
									convolve3x3(&input, &w, wi + 27),
								);

								let tc0 = t0 + c0;
								*temp.get_unchecked_mut(j) = tc0.extract(0);
								*temp.get_unchecked_mut(j + 1) = tc0.extract(1);
								*temp.get_unchecked_mut(j + 2) = tc0.extract(2);
								*temp.get_unchecked_mut(j + 3) = tc0.extract(3);
							}

							{
								let t0 = f32x4::new(
									*temp.get_unchecked(j + 4),
									*temp.get_unchecked(j + 5),
									*temp.get_unchecked(j + 6),
									*temp.get_unchecked(j + 7),
								);
								let c0 = f32x4::new(
									convolve3x3(&input, &w, wi + 36),
									convolve3x3(&input, &w, wi + 45),
									convolve3x3(&input, &w, wi + 54),
									convolve3x3(&input, &w, wi + 63),
								);

								let tc0 = t0 + c0;
								*temp.get_unchecked_mut(j + 4) = tc0.extract(0);
								*temp.get_unchecked_mut(j + 5) = tc0.extract(1);
								*temp.get_unchecked_mut(j + 6) = tc0.extract(2);
								*temp.get_unchecked_mut(j + 7) = tc0.extract(3);
							}
						} else {
	...
	unsafe {
		if USE_SIMD {
...

			{
				...
				let i1 = f32x4::new(
					*w.get_unchecked(off),
					*w.get_unchecked(off + 1),
					*w.get_unchecked(off + 2),
					*w.get_unchecked(off + 3),
				);

				let i01 = i0 * i1;

				sum = i01.extract(0) + i01.extract(1) + i01.extract(2) + i01.extract(3);
			}

			{
				...
				let i1 = f32x4::new(
					*w.get_unchecked(off + 4),
					*w.get_unchecked(off + 5),
					*w.get_unchecked(off + 6),
					*w.get_unchecked(off + 7),
				);

				let i01 = i0 * i1;

				sum = sum + i01.extract(0) + i01.extract(1) + i01.extract(2) + i01.extract(3);
			}

			sum + *input.get_unchecked(8) * *w.get_unchecked(off + 8)
		} else {
			*input.get_unchecked(0) * *w.get_unchecked(off) + *input.get_unchecked(1) * *w.get_unchecked(off + 1)
...
#[inline(always)]
fn filter_cpu2_layer_bias_relu(
	layer: &Layer,
	num_out: usize,
	x: usize,
	temp: &mut [f32; 128],
...
#[inline(always)]
fn filter_cpu2_layer_writeback_line(
	num_out: usize,
	width: usize,
	y: usize,
	stride: usize,
